...
We will see a quick setup to configure the ``backupninja`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``backupninja`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``backupninja`` formulas under ``/srv/salt/backupninja/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/backupninja/parameters/dns:domain/
    ln -s dns:domain /srv/salt/backupninja/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/backupninja/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/backupninja/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``backupninja`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``backupninja`` formulas under ``/srv/salt/backupninja/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/backupninja/parameters/roles
...
We create a configuration for the role ``backupninja/server`` in ``/srv/salt/backupninja/parameters/roles/backupninja/server.yaml``:
...
We create another configuration for the role ``backupninja/client`` in ``/srv/salt/backupninja/parameters/roles/backupninja/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles backupninja/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles backupninja/client
...
.. code-block:: console
    ...
    mv /srv/salt/backupninja/parameters/dns:domain/  '/srv/salt/backupninja/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-backupninja-as-json:
      test.nop:
        - name: {{ backupninja | json }}
...
Here is an example based on `template-formula/backupninja/config/file.sls`_:
...
.. code-block:: sls
    ...
    backupninja-config-file-file-managed:
      file.managed:
        - name: {{ backupninja.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='backupninja-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        ...
        - context:
            backupninja: {{ backupninja | json }}

This ``sls`` file expose a ``backupninja`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ backupninja['winner'] }}
...
.. _template-formula/backupninja/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/backupninja/config/file.sls
